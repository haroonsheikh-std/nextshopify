...
const sessionStorage = new RedisStore(process.env.NEXT_PUBLIC_REDIS_URL);
const context = {
  API_KEY: process.env.NEXT_PUBLIC_SHOPIFY_APP_API_KEY,
  API_SECRET_KEY: process.env.NEXT_PUBLIC_SHOPIFY_APP_API_SECRET_KEY,
  SCOPES: [process.env.NEXT_PUBLIC_SCOPES || 'write_products'],
  HOST_NAME: process.env.NEXT_PUBLIC_HOST || 'https://example.com',
...
export function ShopifyAuth(config = {}) {
  ...
  return (req, res) => {
    ...
    console.log("consoleeeee", {shopify})//console
    ...
    console.log('shopify auth',req.headers)//console
...
async function loginRoute(req, res) {
  try {
    const shop = "boxbuilder-dev.myshopify.com";
    console.log("request handler", req)//console
    console.log('shopssssss', shop)
    const authRoute = await Shopify.Auth.beginAuth(req, res, shop, '/api/auth/shopify/callback', true);
    console.log("New OAuth process begining.", authRoute)
